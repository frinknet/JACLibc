...
TEST(ip_types_n_short) {
	ASSERT_SIZE_SAME(n_short, uint16_t);
}

TEST(ip_types_n_long) {
	ASSERT_SIZE_SAME(n_long, uint32_t);
}

TEST(ip_types_n_time) {
	ASSERT_SIZE_SAME(n_time, uint32_t);
}
...
TEST(ipv4_vers_ihl_construct) {
	ASSERT_EQ(IPV4_VERS_IHL(4, 5), 0x45);
}

TEST(ipv4_vers_ihl_mask_hl) {
	ASSERT_EQ(IPV4_VERS_IHL(6, 15), 0x6F);
}

/* ============================================================================ */

TEST_SUITE(ipv4_version_extract);

TEST(ipv4_version_extract) {
	ASSERT_EQ(IPV4_VERSION(0x45), 4);
}

/* ============================================================================ */

TEST_SUITE(ipv4_ihl_extract);

TEST(ipv4_ihl_extract) {
...
TEST(ipv4_hdr_size) {
	ASSERT_EQ(sizeof(struct ipv4_hdr), 20);
}
...
TEST(ipv4_hdr_alignment) {
	ASSERT_SIZE_ALIGNS(struct ipv4_hdr, uint32_t);
}
...
TEST(ip_size) {
	ASSERT_SIZE_SAME(struct ipv4_hdr, struct ip);
}
...
TEST_MAIN_IF(JACL_HAS_POSIX, "net/ipv4.h requires POSIX\n")
