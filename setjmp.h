...
/* setjmp/longjmp */
static inline int setjmp(jmp_buf env) { return __builtin_setjmp(env); }
static inline void longjmp(jmp_buf env, int val) { __builtin_longjmp(env, val); }

/* sigsetjmp/siglongjmp: if save==0, behave like setjmp */
static inline int sigsetjmp(sigjmp_buf env, int save) {
