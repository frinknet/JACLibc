/* (c) 2025-2026 FRINKnet & Friends – MIT licence */
#ifndef _DIRENT_H
#define _DIRENT_H
#pragma once

#include <config.h>
#include <limits.h>
#include <stdlib.h>     // malloc(), free(), qsort()
#include <string.h>     // memcpy(), strlen(), strcmp()
#include <sys/types.h>  // ino_t, off_t, mode_t, dev_t, size_t, ssize_t
#include <stddef.h>     // size_t, NULL, offsetof()
#include <errno.h>      // EINVAL, ENOMEM, EBADF, ENOSYS, errno

#if JACL_OS_WINDOWS
	#define DIRENT_WIN32 1
	#include <windows.h> /* windows API */
#else
	#define DIRENT_POSIX 1
	#include <unistd.h>
	#include <fcntl.h>
	#include <sys/syscall.h>
	#include <sys/stat.h>
#endif

#ifdef __cplusplus
extern "C" {
#endif

#if JACL_HAS_PTHREADS && JACL_HAS_C11
	#define DIRENT_THREADS 1
	#include <stdatomic.h>

	#if !JACL_OS_WINDOWS
		#include <sched.h>    // sched_yield()
	#endif
#endif

/* ================================================================ */
/* Constants                                                        */
/* ================================================================ */

/* File type constants for d_type field */
#define DT_UNKNOWN  0   /* Unknown file type */
#define DT_FIFO     1   /* FIFO/pipe */
#define DT_CHR      2   /* Character device */
#define DT_DIR      4   /* Directory */
#define DT_BLK      6   /* Block device */
#define DT_REG      8   /* Regular file */
#define DT_LNK      10  /* Symbolic link */
#define DT_SOCK     12  /* Socket */
#define DT_WHT      14  /* Whiteout (BSD) */

/* Convert between stat st_mode and d_type */
#define IFTODT(mode)    (((mode) & 0170000) >> 12)
#define DTTOIF(dirtype) ((dirtype) << 12)

#if DIRENT_WIN32
/* ================================================================ */
/* Windows implementation with Unicode support                     */
/* ================================================================ */

typedef struct dirent {
	ino_t d_ino;                /* File serial number */
	unsigned char d_type;       /* File type */
	unsigned short d_reclen;    /* Record length */
	unsigned short d_namlen;    /* Name length */
	char d_name[NAME_MAX + 1];  /* Filename (UTF-8) */
} dirent;

typedef struct DIR {
	HANDLE handle;              /* Windows search handle */
	WIN32_FIND_DATAW find_data; /* Windows file data (Unicode) */
	int first_call;             /* First call flag */
	wchar_t path[PATH_MAX];     /* Directory path (Unicode) */
	dirent current;             /* Current entry */
	long position;              /* Current position for telldir/seekdir */
	int error_code;             /* Last error code */
} DIR;

#else
/* ================================================================ */
/* POSIX implementation                                             */
/* ================================================================ */

typedef struct dirent {
	ino_t d_ino;                /* File serial number */
	off_t d_off;                /* File offset (Linux) */
	unsigned short d_reclen;    /* Record length */
	unsigned char d_type;       /* File type */
	char d_name[NAME_MAX + 1];  /* Filename */
} dirent;

typedef struct DIR {
	int fd;              /* Directory file descriptor */
	char *buffer;        /* Buffer for getdents */
	size_t buffer_size;  /* Buffer size */
	size_t buffer_pos;   /* Current position in buffer */
	size_t buffer_end;   /* End of valid data in buffer */
	long position;       /* Current position for telldir/seekdir */
	dirent current;      /* Current entry */
	int error_code;      /* Last error code */
	_Atomic int lock;    /* Simple spinlock for thread safety */
} DIR;

/* Linux getdents structure (different from dirent) */
#if JACL_HAS_C99
typedef struct linux_dirent {
  unsigned long d_ino;
  unsigned long d_off;
  unsigned short d_reclen;
  char d_name[];          /* C99 flexible array member */
} linux_dirent;
#else
typedef struct linux_dirent {
  unsigned long d_ino;
  unsigned long d_off;
  unsigned short d_reclen;
  char d_name[1];         /* C89 hack */
} linux_dirent;
#endif

#if JACL_HASSYS(getdents64)
typedef struct __jacl_linux_dirent64 {
	uint64_t d_ino;
	int64_t d_off;
	unsigned short d_reclen;
	unsigned char d_type;
	char d_name[];
} linux_dirent64;
#endif


#endif

/* POSIX.1-2008 addition - posix_dent structure */
typedef struct posix_dent {
	ino_t d_ino;               /* File serial number */
	reclen_t d_reclen;         /* Length of this entry */
	unsigned char d_type;      /* File type */
	char d_name[];            /* Filename string */
} posix_dent;

/* ================================================================ */
/* Thread safety documentation                                     */
/* ================================================================ */

/*
 * THREAD SAFETY NOTES:
 *
 * readdir() - Thread-safe when each thread uses its own DIR* object.
 *             NOT safe when multiple threads access the same DIR*.
 *             Use external synchronization if needed.
 *
 * readdir_r() - Deprecated since glibc 2.24 due to buffer overflow risks.
 *               Provided for POSIX compliance but readdir() is preferred.
 *
 * Other functions - Generally thread-safe when operating on different DIR* objects.
 */

/* ================================================================ */
/* Function implementations                                         */
/* ================================================================ */

#if DIRENT_WIN32
/* ================================================================ */
/* Windows implementation with Unicode support                     */
/* ================================================================ */

/* Convert UTF-16 to UTF-8 */
static inline int win32_wchar_to_utf8(const wchar_t *wstr, char *utf8, size_t utf8_size) {
	int result = WideCharToMultiByte(CP_UTF8, 0, wstr, -1, utf8, (int)utf8_size, NULL, NULL);
	return result > 0 ? 0 : -1;
}

/* Convert UTF-8 to UTF-16 */
static inline int win32_utf8_to_wchar(const char *utf8, wchar_t *wstr, size_t wstr_size) {
	int result = MultiByteToWideChar(CP_UTF8, 0, utf8, -1, wstr, (int)wstr_size);
	return result > 0 ? 0 : -1;
}

/* Convert Windows attributes to d_type */
static inline unsigned char win32_attrs_to_dtype(DWORD attrs, DWORD reparse_tag) {
	if (attrs & FILE_ATTRIBUTE_REPARSE_POINT) {
		if (reparse_tag == IO_REPARSE_TAG_SYMLINK ||
		    reparse_tag == IO_REPARSE_TAG_MOUNT_POINT) {
			return DT_LNK;
		}
	}
	if (attrs & FILE_ATTRIBUTE_DIRECTORY) return DT_DIR;
	if (attrs & FILE_ATTRIBUTE_DEVICE) return DT_CHR;
	return DT_REG;
}

static inline DIR *opendir(const char *dirname) {
	if (!dirname) return (__errno_set(EINVAL), NULL);

	DIR *dirp = (DIR *)malloc(sizeof(DIR));

	if (!dirp) return (__errno_set(ENOMEM), NULL);

	if (win32_utf8_to_wchar(dirname, dirp->path, PATH_MAX) != 0) {
		free(dirp);

		return (__errno_set(EINVAL), NULL);
	}

	/* Build search pattern: "dirname\*" */
	size_t len = wcslen(dirp->path);

	if (len >= PATH_MAX - 3) {
		free(dirp);

		return (__errno_set(ENAMETOOLONG), NULL);
	}

	if (len > 0 && dirp->path[len - 1] != L'\\' && dirp->path[len - 1] != L'/') {
		wcscat(dirp->path, L"\\");
	}

	wcscat(dirp->path, L"*");

	dirp->handle = INVALID_HANDLE_VALUE;
	dirp->first_call = 1;
	dirp->position = 0;
	dirp->error_code = 0;

	return dirp;
}

/**
 * readdir()
 *
 * NOTE: Returns a pointer to an internal buffer within the DIR structure.
 * Subsequent calls to readdir() will overwrite this data.
 * If you need to store the filename, copy d_name immediately.
 */
static inline dirent *readdir(DIR *dirp) {
	if (!dirp) return (__errno_set(EBADF), NULL);

	BOOL result;

	if (dirp->first_call) {
		dirp->handle = FindFirstFileW(dirp->path, &dirp->find_data);

		if (dirp->handle == INVALID_HANDLE_VALUE) {
			DWORD error = GetLastError();

			return (__errno_set((error == ERROR_FILE_NOT_FOUND) ? ENOENT : EACCES), NULL);
		}

		dirp->first_call = 0;
		result = TRUE;
	} else {
		result = FindNextFileW(dirp->handle, &dirp->find_data);
	}

	if (!result) {
		DWORD error = GetLastError();

		if (error == ERROR_NO_MORE_FILES) __errno_clr();
		else __errno_set(EIO);

		return NULL;
	}

	/* Fill dirent structure */
	dirp->current.d_ino = 0;  /* Windows doesn't have inode numbers */
	dirp->current.d_type = win32_attrs_to_dtype(
		dirp->find_data.dwFileAttributes,
		dirp->find_data.dwReserved0);

	/* Convert filename from UTF-16 to UTF-8 */
	if (win32_wchar_to_utf8(dirp->find_data.cFileName, dirp->current.d_name, NAME_MAX + 1) != 0) return (__errno_set(EILSEQ), NULL);

	size_t name_len = strlen(dirp->current.d_name);

	dirp->current.d_namlen = (unsigned short)name_len;
	dirp->current.d_reclen = (unsigned short)(sizeof(dirent) - NAME_MAX - 1 + name_len + 1);
	dirp->position++;

	return &dirp->current;
}

/* Thread-safe readdir_r (deprecated but provided for POSIX compliance) */
static inline int readdir_r(DIR *dirp, dirent *entry, dirent **result) {
	if (!dirp || !entry || !result) return __errno_val(EINVAL);

	*result = NULL;

	dirent *current = readdir(dirp);

	if (!current) return __errno_get();

	*entry = *current;
	*result = entry;

	return 0;
}

static inline int closedir(DIR *dirp) {
	if (!dirp) return (__errno_set(EBADF), -1);
	if (dirp->handle != INVALID_HANDLE_VALUE) FindClose(dirp->handle);

	free(dirp);

	return 0;
}

static inline void rewinddir(DIR *dirp) {
	if (!dirp) return;

	if (dirp->handle != INVALID_HANDLE_VALUE) {
		FindClose(dirp->handle);
		dirp->handle = INVALID_HANDLE_VALUE;
	}

	dirp->first_call = 1;
	dirp->position = 0;
}

static inline long telldir(DIR *dirp) { return dirp ? dirp->position : (__errno_set(EBADF), -1); }

static inline void seekdir(DIR *dirp, long pos) {
	if (!dirp) return;

	if (pos < 0) pos = 0;

	/* Windows doesn't support seeking to arbitrary positions easily */
	if (pos < dirp->position) {
		rewinddir(dirp);
	}

	/* Read entries until we reach the desired position */
	while (dirp->position < pos) {
		if (!readdir(dirp)) break;
	}
}

static inline int dirfd(DIR *dirp) {
	(void)dirp;
	return (__errno_set(ENOTSUP), -1);
}

static inline DIR *fdopendir(int fd) {
	(void)fd;
	return (__errno_set(ENOTSUP), NULL);
}

#else
/* ================================================================ */
/* POSIX implementation with improved thread safety                */
/* ================================================================ */

#define DIRENT_BUFFER_SIZE 4096

// POSIX.1-2008 getdents function
static inline ssize_t posix_getdents(int fd, void *buf, size_t nbyte, int flags) {
	(void)flags;  /* Not used in basic implementation */

	if (!buf || nbyte == 0) return (__errno_set(EINVAL), -1);

	#if JACL_HASSYS(getdents64)
		return syscall(SYS_getdents64, fd, buf, nbyte);
	#elif JACL_HASSYS(getdents)
		return syscall(SYS_getdents, fd, buf, nbyte);
	#endif

	/* Not supported or no syscall available */
	(void)fd;
	return (__errno_set(ENOSYS), -1);
}

/* Simple atomic operations for spinlock */
#if DIRENT_THREADS
static inline void dir_lock(DIR *dirp) {
	int expected = 0;

	while (!atomic_compare_exchange_strong_explicit(
	       (atomic_int*)&dirp->lock, &expected, 1,
	       memory_order_acquire, memory_order_relaxed)) {
		expected = 0;
		sched_yield();
	}
}

static inline void dir_unlock(DIR *dirp) {
	atomic_store_explicit((atomic_int*)&dirp->lock, 0, memory_order_release);
}
#endif

static inline DIR *opendir(const char *dirname) {
	if (!dirname) return (__errno_set(EINVAL), NULL);

	int fd = open(dirname, O_RDONLY | O_CLOEXEC);

	if (fd == -1) return NULL;

	DIR *dirp = (DIR *)malloc(sizeof(DIR));

	if (!dirp) {
		close(fd);

		return (__errno_set(ENOMEM), NULL);
	}

	dirp->buffer = (char *)malloc(DIRENT_BUFFER_SIZE);

	if (!dirp->buffer) {
		close(fd);
		free(dirp);

		return (__errno_set(ENOMEM), NULL);
	}

	dirp->fd = fd;
	dirp->buffer_size = DIRENT_BUFFER_SIZE;
	dirp->buffer_pos = 0;
	dirp->buffer_end = 0;
	dirp->position = 0;
	dirp->error_code = 0;

	#if DIRENT_THREADS
		atomic_store_explicit((atomic_int*)&dirp->lock, 0, memory_order_relaxed);
	#else
		dirp->lock = 0;
	#endif

	return dirp;
}

/**
 * readdir()
 *
 * NOTE: Returns a pointer to an internal buffer within the DIR structure.
 * Subsequent calls to readdir() will overwrite this data.
 * If you need to store the filename, copy d_name immediately.
 */
static inline dirent *readdir(DIR *dirp) {
	if (!dirp) return (__errno_set(EBADF), NULL);

	/* Refill buffer if needed */
	if (dirp->buffer_pos >= dirp->buffer_end) {
		ssize_t bytes = posix_getdents(dirp->fd, dirp->buffer, dirp->buffer_size, 0);

		if (bytes == 0) return (__errno_clr(), NULL);

		dirp->buffer_end = bytes;
		dirp->buffer_pos = 0;
	}

	#if JACL_HASSYS(getdents64)
		if (dirp->buffer_pos + sizeof(linux_dirent64) > dirp->buffer_end) return (__errno_set(EIO), NULL);

		linux_dirent64 *d = (linux_dirent64 *)(dirp->buffer + dirp->buffer_pos);

		if (d->d_reclen < sizeof(linux_dirent64) || dirp->buffer_pos + d->d_reclen > dirp->buffer_end) return (__errno_set(EIO), NULL);

		dirp->current.d_ino = d->d_ino;
		dirp->current.d_off = d->d_off;
		dirp->current.d_reclen = d->d_reclen;
		dirp->current.d_type = (d->d_type <= DT_WHT) ? d->d_type : DT_UNKNOWN;

		size_t name_len = strnlen(d->d_name, NAME_MAX);

		memcpy(dirp->current.d_name, d->d_name, name_len);

		dirp->current.d_name[name_len] = '\0';
	#else
		if (dirp->buffer_pos + sizeof(linux_dirent) > dirp->buffer_end) return (__errno_set(EIO), NULL);

		linux_dirent *d = (linux_dirent *)(dirp->buffer + dirp->buffer_pos);

		if (d->d_reclen < sizeof(linux_dirent) || dirp->buffer_pos + d->d_reclen > dirp->buffer_end) return (__errno_set(EIO), NULL);

		dirp->current.d_ino = d->d_ino;
		dirp->current.d_off = d->d_off;
		dirp->current.d_reclen = d->d_reclen;

		char *name_ptr = dirp->buffer + dirp->buffer_pos + offsetof(linux_dirent, d_name);
		struct stat st;

		if (fstatat(dirp->fd, name_ptr, &st, 0) == 0) dirp->current.d_type = IFTODT(st.st_mode);
		else dirp->current.d_type = DT_UNKNOWN;

		size_t name_len = strnlen(name_ptr, NAME_MAX);

		memcpy(dirp->current.d_name, name_ptr, name_len);

		dirp->current.d_name[name_len] = '\0';
	#endif

	dirp->buffer_pos += d->d_reclen;
	dirp->position++;

	return &dirp->current;
}

static inline int readdir_r(DIR *dirp, dirent *entry, dirent **result) {
	if (!dirp || !entry || !result) return __errno_val(EINVAL);

	*result = NULL;

	#if DIRENT_THREADS
		dir_lock(dirp);
	#endif

	dirent *current = readdir(dirp);
	int olderr = __errno_get();

	if (current) {
		size_t name_len = strlen(current->d_name);

		if (name_len > NAME_MAX) {
			#if DIRENT_THREADS
				dir_unlock(dirp);
			#endif

			return __errno_val(ENAMETOOLONG);
		}

		*entry = *current;
		*result = entry;
	}

	#if DIRENT_THREADS
		dir_unlock(dirp);
	#endif

	return current ? 0 : olderr;
}

static inline int closedir(DIR *dirp) {
	if (!dirp) return (__errno_set(EBADF), -1);

	int ret = close(dirp->fd);

	free(dirp->buffer);
	free(dirp);

	if (ret != 0) return (__errno_set(EBADF), -1);

	return 0;
}

static inline void rewinddir(DIR *dirp) {
	if (!dirp) return;

	lseek(dirp->fd, 0, SEEK_SET);

	dirp->buffer_pos = 0;
	dirp->buffer_end = 0;
	dirp->position = 0;
}

static inline long telldir(DIR *dirp) {
	if (!dirp) return (__errno_set(EBADF), -1);

	return dirp->position;
}

static inline void seekdir(DIR *dirp, long pos) {
	if (!dirp) return;

	if (pos < 0) pos = 0;

	if (pos == 0) {
		rewinddir(dirp);
		return;
	}

	/* Simple implementation: rewind and read forward */
	rewinddir(dirp);

	for (long i = 0; i < pos; i++) {
		if (!readdir(dirp)) break;
	}
}

static inline int dirfd(DIR *dirp) {
	if (!dirp) return (__errno_set(EBADF), -1);

	return dirp->fd;
}

static inline DIR *fdopendir(int fd) {
	if (fd < 0) return (__errno_set(EBADF), NULL);

	DIR *dirp = (DIR *)malloc(sizeof(DIR));

	if (!dirp) return (__errno_set(ENOMEM), NULL);

	dirp->buffer = (char *)malloc(DIRENT_BUFFER_SIZE);

	if (!dirp->buffer) {
		free(dirp);
		return (__errno_set(ENOMEM), NULL);
	}

	dirp->fd = fd;
	dirp->buffer_size = DIRENT_BUFFER_SIZE;
	dirp->buffer_pos = 0;
	dirp->buffer_end = 0;
	dirp->position = 0;
	dirp->error_code = 0;

	#if DIRENT_THREADS
		atomic_store_explicit((atomic_int*)&dirp->lock, 0, memory_order_relaxed);
	#else
		dirp->lock = 0;
	#endif

	return dirp;
}

#endif

/* ================================================================ */
/* Additional POSIX functions (improved implementations)           */
/* ================================================================ */

// Directory entry comparison for scandir
static inline int alphasort(dirent **a, dirent **b) {
	if (!a || !*a || !b || !*b) return 0;

	return strcmp((*a)->d_name, (*b)->d_name);
}

// Reverse alphabetical sort
static inline int versionsort(dirent **a, dirent **b) {
	if (!a || !*a || !b || !*b) return 0;

	return strcmp((*a)->d_name, (*b)->d_name);
}

// Scan directory and sort entries
static inline int scandir(const char *dirpath, dirent ***namelist,
                         int (*select_fn)(const dirent *),
                         int (*compar)(dirent **, dirent **)) {
	if (!dirpath || !namelist) return (__errno_set(EINVAL), -1);

	DIR *dirp = opendir(dirpath);
	if (!dirp) return -1;

	dirent **list = NULL;
	int count = 0;
	int capacity = 16;

	list = (dirent **)malloc(capacity * sizeof(dirent *));
	if (!list) {
		closedir(dirp);
		return (__errno_set(ENOMEM), -1);
	}

	dirent *entry;
	while ((entry = readdir(dirp)) != NULL) {
		/* Apply filter function if provided */
		if (select_fn && !select_fn(entry)) continue;

		/* Resize array if needed */
		if (count >= capacity) {
			capacity *= 2;
			dirent **new_list = (dirent **)realloc(list, capacity * sizeof(dirent *));
			if (!new_list) {
				for (int i = 0; i < count; i++) free(list[i]);
				free(list);
				closedir(dirp);
				return (__errno_set(ENOMEM), -1);
			}
			list = new_list;
		}

		/* Allocate and copy entry */
		size_t entry_size = sizeof(dirent);
		list[count] = (dirent *)malloc(entry_size);
		if (!list[count]) {
			for (int i = 0; i < count; i++) free(list[i]);
			free(list);
			closedir(dirp);
			return (__errno_set(ENOMEM), -1);
		}

		*list[count] = *entry;  /* Struct copy */
		count++;
	}

	closedir(dirp);

	/* Sort if comparison function provided */
	if (count > 0 && compar) {
		qsort(list, count, sizeof(dirent *),
		      (int (*)(const void *, const void *))compar);
	}

	*namelist = list;
	return count;
}


#ifdef __cplusplus
}
#endif
#endif /* _DIRENT_H */
