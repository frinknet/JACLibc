 ...
 * - Pure transport: No buffer, no encryption flags, no compression. Just bytes in/out.
 */
...
typedef enum {
	CONN_STREAM,    /* TCP, Unix stream - ordered, reliable byte stream */
	CONN_DGRAM,     /* UDP, Unix datagram - message boundaries preserved */
	CONN_PACKET     /* Raw packet - Ethernet/IP level */
} conn_type_t;

typedef struct conn *conn_t;
typedef struct conn_ops *conn_ops_t;
...
/**
 * Connection handle - PURE TRANSPORT
 *
 * MEMORY OWNERSHIP:
 * - conn_create(), conn_from_fd(): Library allocates (malloc).
 * - conn_close(): Library frees (free) after closing FD (if owns_fd=true).
 * - User NEVER calls free() on conn_t directly.
 *
 * NO BUFFER: Buffer lives in serve_conn_t / client_conn_t.
 * NO ENCRYPTION FLAGS: void *ctx is escape hatch for TLS/SSH when built.
 * NO COMPRESSION: Protocol handlers decide compression semantics.
 */
struct conn {
	conn_type_t type;           /* STREAM, DGRAM, PACKET */
	int domain;                 /* AF_INET, AF_INET6, AF_UNIX */
	int socktype;               /* SOCK_STREAM, SOCK_DGRAM */
	int protocol;               /* IPPROTO_TCP, IPPROTO_UDP */
	int fd;                     /* Underlying file descriptor */
	void *ctx;                  /* Transport-specific context (SSL*, ssh_session, &fd, etc.) */
	conn_ops_t ops;             /* Vtable for extensible I/O */
...
static inline int _conn_std_pending(void *ctx) {
	if (!ctx) { errno = EBADF; return -1; }
	int fd = *(int *)ctx, val = 0;
	if (ioctl(fd, FIONREAD, &val) < 0) return -1;
	return (val > 0) ? 1 : 0;
}

static const struct conn_ops _conn_stream_ops = {
	_conn_std_read,
	_conn_std_write,
	NULL,
	NULL,
...
static const struct conn_ops _conn_dgram_ops = {
	_conn_std_read,
	_conn_std_write,
	_conn_dgram_recvfrom,
	_conn_dgram_sendto,
	_conn_std_close,
	_conn_std_pending,
	NULL,
...
static inline conn_t conn_create(int domain, int type, int protocol) {
	...
	conn_t c = (conn_t)malloc(sizeof(struct conn));
...
static inline int conn_close(conn_t c) {
	...
	/* Only close fd if we own it (prevents double-close on wrappers) */
	if (c->owns_fd && c->fd >= 0 && c->ops->close) {
	    r = c->ops->close(c->ctx);
	}

	/* Null out to prevent use-after-close */
	c->fd = -1;
	c->ops = NULL;
