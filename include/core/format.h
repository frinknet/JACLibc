...
static inline uintptr_t __jacl_get_signed(__jacl_fmt_t spec, va_list ap) {
	...
	#endif
	default: return (uintptr_t)va_arg(ap, int);
	}
}
static inline uintptr_t __jacl_get_unsigned(__jacl_fmt_t spec, va_list ap) {
	switch (JACL_FMT_GET(LENGTH, spec)) {
...
static inline void __jacl_set_unsigned(__jacl_fmt_t spec, va_list ap, uintptr_t v) {
	switch (JACL_FMT_GET(LENGTH, spec)) {
...
// ============ OUTPUT FUNCTIONS ============
static inline int __jacl_output_str(char **out, size_t *rem, __jacl_fmt_t spec, const char *str, int prec, int width) {
	...
	if (!JACL_FMT_HAS(FLAG, spec, left)) __jacl_write_pad(len, out, rem, ' ', pad);
	while (pos < slen) __jacl_write_char(len, out, rem, str[pos++]);
	if (JACL_FMT_HAS(FLAG, spec, left)) __jacl_write_pad(len, out, rem, ' ', pad);
...
static inline int __jacl_output_int(char **out, size_t *rem, const __jacl_fmt_t spec, uintptr_t val, int prec, int width) {
	...
	if (!(prec == 0 && val == 0)) {
		...
		default:
			while (val >= 100) {
				#if JACL_16BIT
					exp = (uintptr_t)(((uint32_t)val * 0xA3D7U) >> 22);
				#else
					exp = (uintptr_t)(((uint64_t)val * 0x51EB851FULL) >> 37);
				#endif
				num = 2 * (val - exp * 100);
				buf[pos++] = D100[num + 1];
				buf[pos++] = D100[num];
			...
			if (val >= 10) { buf[pos++] = D100[val * 2 + 1]; buf[pos++] = D100[val * 2]; }
			else buf[pos++] = '0' + val;
		}
	}
	...
	if (!out || !*out) return pos + sign + prefix + pad;
	if (!JACL_FMT_HAS(FLAG, spec, left) && !(JACL_FMT_HAS(FLAG, spec, zpad) && (prec < 0))) __jacl_write_pad(len, out, rem, ' ', pad);
	if (sign) __jacl_write_sign(len, out, rem, spec, neg);
	if (prefix) { __jacl_write_char(len, out, rem, '0'); if (prefix == 2) __jacl_write_char(len, out, rem, JACL_FMT_HAS(FLAG, spec, upper) ? (JACL_FMT_CHK(BASE, spec, hex) ? 'X' : 'B') : (JACL_FMT_CHK(BASE, spec, hex) ? 'x' : 'b')); }
	if (!JACL_FMT_HAS(FLAG, spec, left) && (JACL_FMT_HAS(FLAG, spec, zpad) && (prec < 0))) __jacl_write_pad(len, out, rem, '0', pad);
	while (pos--) __jacl_write_char(len, out, rem, buf[pos]);
	if (JACL_FMT_HAS(FLAG, spec, left)) __jacl_write_pad(len, out, rem, ' ', pad);

	return len;
}
#undef CASE
static inline int __jacl_output_special(char **out, size_t *rem, const __jacl_fmt_t spec, int val, int neg, int width) {
	...
	if (!out || !*out) return count + pad;
	if (!JACL_FMT_HAS(FLAG, spec, left)) __jacl_write_pad(len, out, rem, ' ', pad);
	if (sign) __jacl_write_sign(len, out, rem, spec, neg);
	if (val == JACL_VAL_NAN) len += __jacl_output_str(out, rem, spec, JACL_FMT_HAS(FLAG, spec, upper) ? "NAN" : "nan", 3, 0);
	if (val == JACL_VAL_INF) len += __jacl_output_str(out, rem, spec, JACL_FMT_HAS(FLAG, spec, upper) ? "INF" : "inf", 3, 0);
	if (JACL_FMT_HAS(FLAG, spec, left)) __jacl_write_pad(len, out, rem, ' ', pad);

	return len;
}
static inline int __jacl_output_float(char **out, size_t *rem, const __jacl_fmt_t spec, long double val, int prec, int width) {
	if (prec < 0) prec = 6;
	...
	if (dec.digits[0] > '9') return __jacl_output_special(out, rem, spec, (val != val) ? JACL_VAL_NAN : JACL_VAL_INF, dec.sign, width);
	if (post < 0) post = 0;
	...
	if (!out || !*out) return total + pad;
	if (!JACL_FMT_HAS(FLAG, spec, left) && !JACL_FMT_HAS(FLAG, spec, zpad)) __jacl_write_pad(len, out, rem, ' ', pad);
	if (sign) __jacl_write_sign(len, out, rem, spec, dec.sign);
	if (!JACL_FMT_HAS(FLAG, spec, left) && JACL_FMT_HAS(FLAG, spec, zpad)) __jacl_write_pad(len, out, rem, '0', pad);
	if (!ipart) __jacl_write_char(len, out, rem, '0');
	else { for (i = 0; i < whole; i++) __jacl_write_char(len, out, rem, dec.digits[dec.start + i]); for (i = whole; i < ipart; i++) __jacl_write_char(len, out, rem, '0'); }
	if (prec > 0 || JACL_FMT_HAS(FLAG, spec, pref)) __jacl_write_char(len, out, rem, '.');

	for (i = 0; i < lead; i++) __jacl_write_char(len, out, rem, '0');
	for (i = 0; i < post; i++) __jacl_write_char(len, out, rem, dec.digits[dec.start + whole + i]);
	for (i = 0; i < trail; i++) __jacl_write_char(len, out, rem, '0');

	if (JACL_FMT_HAS(FLAG, spec, left)) __jacl_write_pad(len, out, rem, ' ', pad);

	return len;
}
static inline int __jacl_output_exp(char **out, size_t *rem, const __jacl_fmt_t spec, long double val, int prec, int width) {
	if (prec < 0) prec = 6;
	...
	__jacl_write_char(len, out, rem, dec.digits[dec.start]);

	if (prec > 0 || JACL_FMT_HAS(FLAG, spec, pref)) __jacl_write_char(len, out, rem, '.');
	for (i = dec.start + 1, remain = prec; i < dec.end && remain > 0; i++, remain--) __jacl_write_char(len, out, rem, dec.digits[i]);
	while (remain-- > 0) __jacl_write_char(len, out, rem, '0');

	__jacl_write_char(len, out, rem, JACL_FMT_HAS(FLAG, spec, upper) ? 'E' : 'e');
	__jacl_write_char(len, out, rem, dec.exp < 0 ? '-' : '+');
	len += __jacl_output_int(out, rem, blank, expval, 2, 0);
...
static inline int __jacl_output_gen(char **out, size_t *rem, const __jacl_fmt_t spec, long double val, int prec, int width) {
	...
	if (isnan(val)) return __jacl_output_special(out, rem, spec, JACL_VAL_NAN, neg, width);
	if (isinf(val)) return __jacl_output_special(out, rem, spec, JACL_VAL_INF, neg, width);
	...
	if (!out || !*out) return sign + pad + ex;

	// Output with padding
	if (!JACL_FMT_HAS(FLAG, spec, left) && !JACL_FMT_HAS(FLAG, spec, zpad)) __jacl_write_pad(len, out, rem, ' ', pad);
	if (sign) __jacl_write_sign(len, out, rem, spec, neg);
	if (!JACL_FMT_HAS(FLAG, spec, left) && JACL_FMT_HAS(FLAG, spec, zpad)) __jacl_write_pad(len, out, rem, '0', pad);
	for (i = 0; i < ex; i++) __jacl_write_char(len, out, rem, buf[i]);
	if (JACL_FMT_HAS(FLAG, spec, left)) __jacl_write_pad(len, out, rem, ' ', pad);

	return len;
}
static inline int __jacl_output_alt(char **out, size_t *rem, const __jacl_fmt_t spec, long double val, int prec, int width) {
	char hex[LDBL_MANT_DIG / 4 + 2];
	...
	if (!out || !*out) return total + padding;

	// Output with sign, prefix, and padding
	if (!JACL_FMT_HAS(FLAG, spec, left) && !JACL_FMT_HAS(FLAG, spec, zpad)) __jacl_write_pad(len, out, rem, ' ', padding);
	if (sign) __jacl_write_sign(len, out, rem, spec, neg);

	__jacl_write_char(len, out, rem, '0');
	__jacl_write_char(len, out, rem, JACL_FMT_HAS(FLAG, spec, upper) ? 'X' : 'x');

	if (!JACL_FMT_HAS(FLAG, spec, left) && JACL_FMT_HAS(FLAG, spec, zpad)) __jacl_write_pad(len, out, rem, '0', padding);

	// Output mantissa
	__jacl_write_char(len, out, rem, '0' + leading);
	...
	if (num_digits || JACL_FMT_HAS(FLAG, spec, pref)) {
		...
		__jacl_write_char(len, out, rem, '.');

		for (i = 0; i < num_digits; i++) __jacl_write_char(len, out, rem, digits_ptr[hex[i]]);
	}

	// Output exponent
	__jacl_write_char(len, out, rem, JACL_FMT_HAS(FLAG, spec, upper) ? 'P' : 'p');
	__jacl_write_char(len, out, rem, (exp < 0) ? '-' : '+');

	if (exponent_absolute >= 100) __jacl_write_char(len, out, rem, '0' + exponent_absolute / 100);
	if (exponent_absolute >= 10) __jacl_write_char(len, out, rem, '0' + (exponent_absolute / 10) % 10);

	__jacl_write_char(len, out, rem, '0' + exponent_absolute % 10);

	if (JACL_FMT_HAS(FLAG, spec, left)) __jacl_write_pad(len, out, rem, ' ', padding);

	return len;
}

// ============ PRINTF FUNCTIONS ============
static inline int __jacl_printf_ptr(char **out, size_t *rem, __jacl_fmt_t spec, va_list ap, int prec, int width) {
	void *p = va_arg(ap, void*);
	union { void *p; uintptr_t u; } u;

	return (u.p = p)
	     ? __jacl_output_int(out, rem, JACL_FMT_SET(BASE, spec, hex) | JACL_FMT_FLAG_pref, u.u, -1, width)
	     : __jacl_output_str(out, rem, spec, "0x0", 3, width);
}
static inline int __jacl_printf_char(char **out, size_t *rem, __jacl_fmt_t spec, va_list ap, int prec, int width) {
	int ch = va_arg(ap, int), pad = (width > 1) ? width - 1 : 0, len = 0;

	if (!JACL_FMT_HAS(FLAG, spec, left)) __jacl_write_pad(len, out, rem, ' ', pad);

	__jacl_write_char(len, out, rem, (char)ch);

	if (JACL_FMT_HAS(FLAG, spec, left)) __jacl_write_pad(len, out, rem, ' ', pad);

	return len;
}
static inline int __jacl_printf_str(char **out, size_t *rem, __jacl_fmt_t spec, va_list ap, int prec, int width) {
	const char *str = va_arg(ap, const char*);

	if (!str) str = "(null)";

	return __jacl_output_str(out, rem, spec, str, prec, width);
}
#define __jacl_printf_base(name) \
static inline int __jacl_printf_##name(char **out, size_t *rem, __jacl_fmt_t spec, va_list ap, int prec, int width) { \
	uintptr_t val = JACL_FMT_HAS(FLAG, spec, sign) ? __jacl_get_signed(spec, ap) : __jacl_get_unsigned(spec, ap); \
	return __jacl_output_int(out, rem, JACL_FMT_SET(BASE, spec, name), val, prec, width); \
}
#define __jacl_printf_style(name) \
static inline int __jacl_printf_##name(char **out, size_t *rem, __jacl_fmt_t spec, va_list ap, int prec, int width) { \
	long double val = __jacl_get_float(spec, ap); \
	return __jacl_output_##name(out, rem, spec, val, prec, width); \
}
...
static inline int __jacl_printf(char * restrict out, size_t n, const char *fmt, va_list ap) {
	if (!fmt) return -1;
	...
	while (*fmt) {
		...
		switch (type) {
		case 'n': __jacl_set_signed(spec, ap, (uintptr_t)len); break;
...
// ============ INPUT FUNCTIONS =============
static inline int __jacl_input_str(const char **in, FILE *stream, int *read, __jacl_fmt_t spec, int width, char *buf) {
	int i = 0, ch, pad = JACL_FMT_HAS(FLAG, spec, spad) ? 1 : 0;

	if (pad) __jacl_read_skip(ch, in, stream, read);
...
static inline int __jacl_input_scan(const char **in, FILE *stream, int *read, const char **fmt, int width, char *buf) {
	char charset[256] = {0};
	...
	while (i < width) {
		...
		if (!match) { __jacl_read_back(ch, in, stream, read); break; }
...
#define __jacl_scanf_base(name) static inline int __jacl_scanf_##name(const char **in, FILE *stream, int *read, __jacl_fmt_t spec, int width, va_list ap) { \
	...
	__jacl_read_skip(ch, in, stream, read); \
	if (width == 0) width = JACL_FMT_WIDTH_num; \
	if (!__jacl_input_int(in, stream, read, JACL_FMT_SET(BASE, spec, name), width, &val)) return 0; \
	if (JACL_FMT_HAS(FLAG, spec, noout)) return 1; \
...
static inline int __jacl_scanf(const char **in, FILE *stream, const char * restrict fmt, va_list ap) {
	...
	while (*fmt) {
		...
		switch (type) {
		case '%': __jacl_read_next(ch, in, stream, &read); if (ch != '%') return (ch == EOF && !matched) ? EOF : count; break;
