...
static inline int __iconv_codepoint_from(int enc, const char **in, size_t *inlen, unsigned int *cp, unsigned int *state) {
	...
	switch (enc) {
		...
		case 1: /* UTF-8 */
			if (state && *state) {
				if ((**in & 0xC0) != 0x80) { *state = 0; return (__errno_set(EILSEQ), -1); }
		...
		case 2: /* UTF-16LE */
			...
			if (u >= 0xD800 && u <= 0xDBFF) {
				...
				if (l < 0xDC00 || l > 0xDFFF) return (__errno_set(EILSEQ), -1);
			...
			if (u >= 0xDC00 && u <= 0xDFFF) return (__errno_set(EILSEQ), -1);
		...
		case 3: /* UTF-32LE */
			...
			*cp = (unsigned char)(*in)[0] | ((unsigned char)(*in)[1] << 8) | ((unsigned char)(*in)[2] << 16) | ((unsigned char)(*in)[3] << 24);
...
static inline int __iconv_codepoint_to(int enc, char **out, size_t *outlen, unsigned int cp) {
	...
	switch (enc) {
		...
		case 1: /* UTF-8 */
			if (cp <= 0x7F) need = 1; else if (cp <= 0x7FF) need = 2; else if (cp <= 0xFFFF) need = 3; else if (cp <= 0x10FFFF) need = 4; else return (__errno_set(EILSEQ), -1);
		...
		case 2: /* UTF-16LE */
			if (cp >= 0x10000 && cp <= 0x10FFFF) {
				...
				if (cp > 0xFFFF) return (__errno_set(EILSEQ), -1);
...
static inline size_t iconv(iconv_t cd, char **restrict inbuf, size_t *restrict inbytesleft, char **restrict outbuf, size_t *restrict outbytesleft) {
	if (!cd || cd == (iconv_t)-1) return (__errno_set(EBADF), (size_t)-1);
	if (!inbuf || !inbytesleft || !outbuf || !outbytesleft) return (__errno_set(EINVAL), (size_t)-1);
	...
	while (*inbytesleft > 0) {
		...
		if (r == 0) { __errno_set(E2BIG); break; }
		if (r < 0) return (size_t)-1;
...
static inline int iconv_close(iconv_t cd) {
	if (!cd || cd == (iconv_t)-1) return (__errno_set(EBADF), -1);
	free(cd);
	return 0;
}
