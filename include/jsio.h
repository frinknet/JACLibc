...
JS_EXPORT("nodeRoot")
js_t *get_root(void) { return JS_ROOT; }

static void js_free(js_t* v);
static void js_notify(js_t* v);

// Node Opperations
JS_EXPORT(nodeAttach)
static void js_attach(js_t* c, js_t* p) {
	if (!c || !p) return;
...
static void js_replace(js_t* o, js_t* n) {
	if (!o || !n) return;
...
// Indexing
static js_t* js_index(js_t* a, int i) {
...
static int js_indexof(js_t* a, js_t* v) {
...
#define JS_BOOLEAN(x) js_number(js_create(JS_TYPE_BOOLEAN,0,0), (double)(x))
...
static void js_unshift(js_t* a, js_t* v) {
