...
static inline void *__jacl_shm_map_file(key_t key, size_t *size, int shmflg) {
	...
	if (fd >= 0 && (shmflg & IPC_EXCL)) {
		...
		return (__errno_set(EEXIST), MAP_FAILED);
	}
	...
	if (fd < 0) {
		...
		if (fd < 0) {
			...
			if (fd >= 0 && (shmflg & IPC_EXCL)) { close(fd); return (__errno_set(EEXIST), MAP_FAILED); }
		...
		if (fstat(fd, &st) == 0) {
			if (*size > 0 && *size > (size_t)st.st_size) {
				close(fd);

				return (__errno_set(EINVAL), MAP_FAILED);
			}
...
static inline int shmget(key_t key, size_t size, int shmflg) {
	if (size == 0 && (shmflg & IPC_CREAT)) return (__errno_set(EINVAL), -1);

	if (key != IPC_PRIVATE) {
		__jacl_shm_seg *existing = __jacl_shm_find_by_key(key);

		if (existing) {
			if ((shmflg & IPC_CREAT) && (shmflg & IPC_EXCL)) return (__errno_set(EEXIST), -1);
			if (size > 0 && size > existing->size) return (__errno_set(EINVAL), -1);

			return existing->id;
		}
	}

	if (!(shmflg & IPC_CREAT)) return (__errno_set(ENOENT), -1);

	__jacl_shm_seg *seg = __jacl_shm_alloc_slot();

	if (!seg) return (__errno_set(ENOSPC), -1);

	void *addr = (key == IPC_PRIVATE) ? mmap(NULL, size, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_ANONYMOUS, -1, 0) : __jacl_shm_map_file(key, &size, shmflg);
...
static inline void *shmat(int shmid, const void *shmaddr, int shmflg) {
	...
	if (!seg) return (__errno_set(EINVAL), (void *)-1);
	if (seg->deleted) return (__errno_set(EIDRM), (void *)-1);
	...
	if (shmaddr) {
		...
		if (shmaddr != seg->addr && !(shmflg & SHM_REMAP)) return (__errno_set(EINVAL), (void *)-1);
	}
...
static inline int shmdt(const void *shmaddr) {
	...
	if (!seg) return (__errno_set(EINVAL), -1);
	if (seg->nattch == 0) return (__errno_set(EINVAL), -1);
	...
	if (seg->nattch == 0 && seg->deleted) __jacl_shm_destroy(seg);
