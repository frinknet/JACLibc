...
static inline size_t __futex_hash(uint32_t *uaddr) { return ((uintptr_t)uaddr >> 2) & (__FUTEX_BUCKETS - 1); }
...
#if JACL_OS_LINUX || JACL_OS_NETBSD || JACL_OS_OPENBSD
/* Native syscall */
static inline int futex(uint32_t *uaddr, int futex_op, uint32_t val, const struct timespec *timeout, uint32_t *uaddr2, uint32_t val3) {
...
static inline int futex(uint32_t *uaddr, int futex_op, uint32_t val, const struct timespec *timeout, uint32_t *uaddr2, uint32_t val3) {
	...
	if (futex_op == FUTEX_WAIT || futex_op == FUTEX_WAIT_BITSET) {
		if (*uaddr != val) { return (__errno_set(EAGAIN), -1); }

		DWORD ms = timeout ? (DWORD)(timeout->tv_sec * 1000ULL + timeout->tv_nsec / 1000000ULL) : INFINITE;

		if (!WaitOnAddress(uaddr, &val, sizeof(uint32_t), ms)) return (__errno_set((GetLastError() == 0x102) ? ETIMEDOUT : EINTR), -1);
	...
	return (__errno_set(ENOSYS),n -1);
}

#else
/* POSIX Fallback: Darwin, FreeBSD, DragonFly, WASM, etc. */
#include <pthread.h>
...
static inline int futex(uint32_t *uaddr, int futex_op, uint32_t val, const struct timespec *timeout, uint32_t *uaddr2, uint32_t val3) {
	...
	if (futex_op == FUTEX_WAIT || futex_op == FUTEX_WAIT_BITSET) {
		...
		if (ret == __errno_get(ETIMEDOUT)) __errno_set(ETIMEDOUT);
