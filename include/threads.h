...
/* Thread functions */
int thrd_create(thrd_t *thr, thrd_start_t func, void *arg) { return pthread_create(thr, NULL, (void*(*)(void*))func, arg); }
...
/* Mutex */
int mtx_init(mtx_t *m, int type) { return pthread_mutex_init(m, NULL); }
...
/* Condition variable */
int cnd_init(cnd_t *c) { return pthread_cond_init(c, NULL); }
int cnd_signal(cnd_t *c) { return pthread_cond_signal(c); }
...
int tss_create(tss_t *key, void (*dtor)(void*)) { return pthread_key_create(key, dtor); }
int tss_delete(tss_t key) { return pthread_key_delete(key); }
int tss_set(tss_t key, void *val) { return pthread_setspecific(key, val) ? errno : 0; }
