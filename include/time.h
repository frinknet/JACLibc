...
// TODO: we need to redefine this between browser and wasm. For the browser we have JS for WASI we have wasi calls
...
static inline time_t time(time_t *tloc) { if (tloc) *tloc = WASM_EPOCH_TIME;return WASM_EPOCH_TIME; }
static inline double difftime(time_t time1, time_t time0) { return (double)(time1 - time0); }
static inline time_t mktime(tm *tm) { (void)tm; return WASM_EPOCH_TIME; }
static inline tm *gmtime(const time_t *timer) { (void)timer; return &wasm_static_tm; }
static inline tm *localtime(const time_t *timer) { (void)timer; return &wasm_static_tm; }
